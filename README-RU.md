...
Любой коммент - это по сути отзыв. Обычно один пользователь оставляет один отзыв к одной статье. Оценки (комменты) к самим отзывам - это другая сущность.

В итоге сущность "ОценкаСтатьи" отправляется на сервер и сохраняется в БД.Для отправки необходима форма.
