...
- FormEntity необходима для заполнения и в основном не содержит данных (за исключением предустановленных фильтров). Пустые поля соответственно не содержат микроразметки. Фильтры (данные формы) зачастую являются частью контента (например, при выборе города - автозаправки города Н).
- Все инпуты помечаются атрибутом name, соответствующим названию поля (свойства) в сущности.
...
Разделение редактирования и отображения
---

- Страница редактирования (создания) сущности (write) (загрузка видео)
  - html form
  - submit button (create, update)
  - authorization rules
- Страница отображения сущности (read) (отображение видео)
  - entity properties
- Страница создания одной сущности (formEntity) и отображения другой (targetEntity)
  - коммент + видео, заказ на услугу + услуга, покупка билета + билет, товар + покупка
  - целевая сущность = mainEntityOfPage
- Страница создания одной сущности и отображение результатов
  - создание тур-заявки + отображение подборки туров
  - каждый фильтр - это перенаправление на конкретную страницу (с подборкой туров, соответствующих новым фильтрам). На этой странице подборки - есть своя форма по выбору конкретного тура (из списка), указание количества, и других данных. По сути две разных формы (форма фильтров, форма выбора конкретного тура с указанием деталей). Форма фильтра должна быть отделена на отдельную страницу (например главная страница с описанием услуги и формой заказа)
